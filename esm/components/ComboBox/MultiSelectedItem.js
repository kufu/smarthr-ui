...
export function MultiSelectedItem({ item, disabled, onDelete, enableEllipsis, buttonRef, decorators, }) {
    ...
    return (React.createElement(MultiSelectedItemTooltip, { needsTooltip: needsTooltip, text: item.label },
        React.createElement(Wrapper, { themes: theme, disabled: disabled, className: classNames.selectedItem },
            React.createElement(ItemLabel, { themes: theme, enableEllipsis: enableEllipsis, className: classNames.selectedItemLabel, ref: labelRef }, item.label),
            deletable && (React.createElement(DeleteButton, { type: "button", themes: theme, className: classNames.deleteButton, disabled: disabled, onClick: () => {
                ...
                React.createElement(FaTimesCircleIcon, { size: 11, color: "inherit", alt: decorators?.destroyButtonIconAlt?.(DESTROY_BUTTON_TEXT) || DESTROY_BUTTON_TEXT }))))));
}
