...
/**
 * react-pdfのDocumentに渡すOptions
 * 描画関数内で定義するとバグるのでトップレベルに静的に配置している
 */
const options = {
  /**
...
/**
 * PDFまたは画像を表示し、回転・拡大縮小の制御を提供するhooks
 * @returns {Object} viewer: ドキュメント表示部分, controller: 制御ボタン群
 */
...
export const FileViewer: FC<Props> = (props) => {
  const { viewer, controller } = useDocumentViewer(props)
  return (
    <div className="shr-flex shr-flex-col shr-w-full shr-gap-1 shr-bg-scrim shr-h-full shr-overflow-auto shr-bg-[radial-gradient(theme(textColor.black)_1px,_transparent_0)] shr-bg-[length:16px_16px]">
