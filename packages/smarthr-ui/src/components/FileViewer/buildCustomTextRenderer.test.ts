...
const callRenderer = (
  ...
  const renderer = buildCustomTextRenderer(matches)
...
describe('buildCustomTextRenderer', () => {
  test('マッチを持たない textItem は素通しでエスケープのみが適用される', () => {
    const result = callRenderer([match({ pageIndex: 0, itemIndex: 0, matchLength: 3 })], {
      pageIndex: 1,
      itemIndex: 0,
      str: `<script>&"'`,
    })
  ...
  test('HTML 特殊文字 (& < > " \') がそれぞれ正しいエンティティに変換される', () => {
    const result = callRenderer([], { pageIndex: 0, itemIndex: 0, str: `&<>"'` })
    expect(result).toBe('&amp;&lt;&gt;&quot;&#39;')
  })

  test('マッチ範囲外と範囲内の両方で HTML 特殊文字がエスケープされる', () => {
    const result = callRenderer([match({ matchStart: 3, matchLength: 5, globalIndex: 0 })], {
      pageIndex: 0,
      itemIndex: 0,
      str: '<a&<a&b>>b',
    })
  ...
  test('マークは選択状態に依らず常に class="highlight" のみで出力される（selected は付与しない）', () => {
    const result = callRenderer([match({ matchStart: 0, matchLength: 3, globalIndex: 5 })], {
      pageIndex: 0,
      itemIndex: 0,
      str: 'abc',
    })
  ...
  test('マッチが textItem の先頭から始まる場合は先頭にエスケープ済みプレフィクスが入らない', () => {
    const result = callRenderer([match({ matchStart: 0, matchLength: 3, globalIndex: 0 })], {
      pageIndex: 0,
      itemIndex: 0,
      str: 'abcdef',
    })
  ...
  test('マッチが textItem の末端で終わる場合は末尾にエスケープ済みサフィクスが入らない', () => {
    const result = callRenderer([match({ matchStart: 3, matchLength: 3, globalIndex: 0 })], {
      pageIndex: 0,
      itemIndex: 0,
      str: 'abcdef',
    })
  ...
  test('複数 textItem を跨ぐ 1 ヒット (同じ globalIndex を共有) でも各 fragment が正しくマークされる', () => {
    ...
    expect(item0).toBe('人<mark class="highlight" data-shr-match-index="0">事</mark>')
    expect(item1).toBe('<mark class="highlight" data-shr-match-index="0">制</mark>度')
  })
})
