...
// HINT: コンポーネントの引数が例えば <AnyTitle /> と { title:  <AnyTitle />, styleType: 'hoge' } のようにObject形式かどうかを判定し
// 常に { title, ...} のようにObject形式に変換するutility
export const convertObjectAttributes = <T, U>(
  original: T,
  callback: (org: Exclude<T, U>) => U,
): U => {
  if (
